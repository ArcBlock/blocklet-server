...
const RuntimeInsightState = require('../../lib/states/runtime-insight');
const { setupInMemoryModels } = require('../../tools/fixture');

describe('RuntimeInsightState', () => {
  let state = null;
  let models = null;

  beforeAll(async () => {
    models = await setupInMemoryModels();
    state = new RuntimeInsightState(models.RuntimeInsight, {});
    state.paginate = mock();
  });
  ...
  test('should call paginate with correct parameters when only did is provided', () => {
    ...
    expect(state.paginate).toHaveBeenCalledWith({ where: { did: 'testDid' } }, { date: -1 }, { pageSize: 360 });
  });
  ...
  test('should call paginate with correct parameters when no parameters are provided', () => {
    state.findPaginated();
    expect(state.paginate).toHaveBeenCalledWith({ where: {} }, { date: -1 }, { pageSize: 360 });
  });
});
