...
const inflate = require('js-git/lib/inflate');
const deflate = require('js-git/lib/deflate');
const codec = require('js-git/lib/object-codec');
const parsePackEntry = require('js-git/lib/pack-codec').parseEntry;
const applyDelta = require('js-git/lib/apply-delta');
const sha1 = require('git-sha1');
const pathJoin = require('path').join;
const pathDir = require('path').dirname;
...
module.exports = (repo, fs) => {
  ...
  function saveRaw(hash, raw, callback) {
    ...
    loadRaw(hash, (err, data) => {
      ...
      const baseDir = pathDir(tmp);
      if (fs.existsSync(baseDir) === false) {
        fs.mkdirSync(pathDir(tmp));
      }
...
function readUint32(buffer, offset) {
  return (
    ((buffer[offset] << 24) | (buffer[offset + 1] << 16) | (buffer[offset + 2] << 8) | (buffer[offset + 3] << 0)) >>> 0
  );
}
