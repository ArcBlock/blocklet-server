...
async function bench(cache, name, totalOps, concurrency) {
  const cacheGroup = Math.random().toString(36).substring(2, 15);
  ...
  for (let i = 0; i < Math.min(100, totalOps); i++) {
    await cache.groupSet(cacheGroup, keys[i], values[i], { ttl: 60000 });
  }

  // SET benchmark
  const setTasks = keys.map((key, i) => () => cache.groupSet(cacheGroup, key, values[i], { ttl: 60000 }));
...
(async () => {
  ...
  const sqliteCache = new DBCache(() => ({ sqlitePath, prefix: 'bench:group:a', ttl: 60000 }));
  await bench(sqliteCache, 'SQLite', totalOps, concurrency);
  await sqliteCache.close();

  if (redisClient) {
    const redisCache = new DBCache(() => ({ sqlitePath, prefix: 'bench:group:b', ttl: 60000, redisUrl }));
