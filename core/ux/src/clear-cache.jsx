...
export default function ClearCache({ teamDid, size = 'small', ...rest }) {
  ...
  const onConfirm = async () => {
    try {
      setLoading(true);
      const result = await api.clearCache({ input: { teamDid } });
      Toast.success(t('blocklet.config.clearCache.success', { count: result.removed?.length || 0 }));
    } catch (err) {
  ...
  return (
    <div {...rest}>
      <Button onClick={() => setConfirmSetting(setting)} color="error" variant="outlined" size={size}>
        ...
        {t('blocklet.config.clearCache.action')}
      </Button>
      {confirmSetting && (
        <Confirm
