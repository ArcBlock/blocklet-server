...
function OAuthGoogle({ ref = null, onSuccess = noop, sortMaps = {} }) {
  const { api } = useNodeContext();
  const { blocklet } = useBlockletContext();
  const { t, locale } = useLocaleContext();
  const did = blocklet?.meta?.did;
  const authData = Object.assign({ ...defaultValue }, blocklet?.settings?.authentication?.google || {});
  const appUrl =
  ...
  const onSubmit = useMemoizedFn(async (data) => {
    try {
      const { blocklet: blockletChanged } = await api.configAuthentication({
        input: {
          ...
          authentication: JSON.stringify({
            ...
            google: {
              ...data,
              order: sortMaps.google?.order ?? Object.keys(sortMaps).length,
              enabled: true,
              type: 'oauth',
            },
          }),
      ...
      const defaultValues = Object.assign({ ...defaultValue }, blockletChanged?.settings?.authentication?.google || {});
  ...
  const validateClientId = useMemoizedFn((val) => {
    if (!val?.trim()) {
      return t('authentication.google.clientIdDescription');
    }
    return true;
  });
  const validateClientSecret = useMemoizedFn((val) => {
    if (!val?.trim()) {
      return t('authentication.google.clientSecretDescription');
    }
    return true;
  });
  ...
  const documentUrl = useCreation(() => {
    if (locale === 'zh') {
      return 'https://www.arcblock.io/blog/docs/did-connect/zh/oauth-config-google';
    }
    return 'https://www.arcblock.io/blog/docs/did-connect/en/oauth-config-google';
  }, [locale]);

  const callbackUrl = joinURL(appUrl, WELLKNOWN_SERVICE_PATH_PREFIX, 'oauth/callback/google');
  ...
  return (
    <Box>
      <form onSubmit={handleSubmit(onSubmit)} disabled>
        ...
        <Section title={t('authentication.google.clientId')}>
          <Controller
            name="clientId"
            control={control}
            ...
            render={({ field }) => (
              <FormControl fullWidth>
                <TextField
                  {...field}
                  size="small"
                  label={t('authentication.google.clientId')}
        ...
        <Section title={t('authentication.google.clientSecret')}>
          <Controller
            name="clientSecret"
            control={control}
            ...
            render={({ field }) => (
              <FormControl fullWidth>
                <TextField
                  ...
                  size="small"
                  type="password"
                  label={t('authentication.google.clientSecret')}
                  error={!!formState.errors[field.name]}
                  helperText={formState.errors[field.name]?.message || '　'}
                />
              </FormControl>
